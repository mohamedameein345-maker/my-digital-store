...
function Store() {
  ...
  useEffect(() => {
    ...
    fetch(`${API_URL}?t=${new Date().getTime()}`)
      ...
      .catch((err) => console.error("خطأ في المنتجات:", err));

    // ملاحظة: هنا مستقبلاً هنضيف fetch لإعدادات الستايل من السيرفر
    // حالياً بنستخدم الإعدادات الافتراضية اللي بتعدلها من لوحة التحكم
  }, []);
  ...
  return (
    <div dir="rtl" style={{ fontFamily: 'Segoe UI, Arial', backgroundColor: '#f0f2f5', minHeight: '100vh' }}>
      ...
      {/* الهيدر المتفاعل مع اللون المختار */}
      <header style={{ backgroundColor: '#fff', padding: '40px 20px', textAlign: 'center', boxShadow: '0 4px 12px rgba(0,0,0,0.05)', borderTop: `5px solid ${siteConfig.mainColor}` }}>
        <h1 style={{ color: siteConfig.mainColor, margin: '0', fontSize: '2.5rem', fontWeight: '800' }}>
          {siteConfig.siteName}
        </h1>
        ...
        {categories.map(cat => (
          <button
            ...
            style={{
              ...
              fontWeight: 'bold',
              transition: '0.3s',
              backgroundColor: activeCategory === cat ? siteConfig.mainColor : '#eee',
              color: activeCategory === cat ? '#fff' : '#555',
              boxShadow: activeCategory === cat ? `0 4px 10px ${siteConfig.mainColor}44` : 'none'
            }}
          >
            {cat}
          </button>
        ...
        {filteredProducts.map((product) => (
          <div key={product._id} style={{ backgroundColor: '#fff', borderRadius: '20px', overflow: 'hidden', boxShadow: '0 10px 30px rgba(0,0,0,0.08)' }}>
            <div style={{ position: 'relative' }}>
              ...
              <span style={{ position: 'absolute', top: '15px', right: '15px', backgroundColor: siteConfig.mainColor, color: 'white', padding: '5px 15px', borderRadius: '15px', fontSize: '12px', fontWeight: 'bold', opacity: 0.9 }}>
            ...
            <div style={{ padding: '25px', textAlign: 'center' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '20px', borderTop: '1px solid #eee', paddingTop: '20px' }}>
                <span style={{ fontSize: '22px', fontWeight: 'bold', color: siteConfig.mainColor }}>{product.price} <small style={{ fontSize: '12px' }}>جنيه</small></span>
                <button 
      ...
      {selectedVideo && (
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', backgroundColor: 'rgba(0,0,0,0.95)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 1000, padding: '20px' }}>
          <div style={{ width: '100%', maxWidth: '900px', position: 'relative' }}>
            <button onClick={() => setSelectedVideo(null)} style={{ position: 'absolute', top: '-50px', right: '0', color: 'white', background: 'none', border: 'none', fontSize: '20px', cursor: 'pointer' }}>إغلاق [X]</button>
            <div style={{ borderRadius: '15px', overflow: 'hidden', backgroundColor: '#000', aspectRatio: '16/9' }}>
      ...
      <footer style={{ textAlign: 'center', padding: '40px', color: '#bdc3c7', backgroundColor: '#fff', marginTop: '50px' }}>
        <p>جميع الحقوق محفوظة لـ {siteConfig.siteName} © 2026</p>
      </footer>
    </div>
  );
}
